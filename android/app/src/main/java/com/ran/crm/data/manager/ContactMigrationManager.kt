            ...
            withContext(Dispatchers.IO) {
                ...
                cursor?.use {
                    ...
                    while (it.moveToNext()) {
                        ...
                        // Use normalized number if available, otherwise fallback to raw or basic
                        // normalization
                        val phoneNormalized =
                        ...
                        if (phoneRaw.isNotBlank() && phoneNormalized.isNotBlank()) {
                            potentialContacts.add(
                                    PotentialContact(name, phoneRaw, phoneNormalized, contactId)
                            )
                        }
                    }
                }
                ...
                if (potentialContacts.isNotEmpty()) {
                    ...
                    phoneList.chunked(900).forEach { chunk ->
                        existingPhones.addAll(contactRepository.getExistingNormalizedPhones(chunk))
                    }
                        ...
                        if (!existingPhones.contains(contact.phoneNormalized)) {
                            ...
                            if (newContact != null) {
                                ...
                                if (deleteAfterImport) {
                                    deleteSystemContact(contentResolver, contact.contactId)
                                }
                            }
