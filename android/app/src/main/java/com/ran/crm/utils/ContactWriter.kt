...
class ContactWriter(
    ...
    data class SyncResult(val exported: Int, val updated: Int, val skipped: Int, val errors: Int)
                    ...
                    for (contact in crmContacts) {
                        try {
                            ...
                            if (rawContactId == null) {
                                ...
                                // Only look for contacts in our account to avoid messing with
                                ...
                                if (rawContactId != null) {
                                    // Found by phone, so we update it and SET the SOURCE_ID for
                                    // future
                                    com.ran.crm.utils.SyncLogger.log(
    ...
    private fun pruneOrphanedContacts(
        ...
        try {
            ...
            for ((rawContactId, sourceId) in accountContacts) {
                ...
                // If we strictly enforce source_id, we should delete it if it wasn't matched during
                // the sync loop above.
                ...
                // If sourceId is NULL, it means it wasn't matched by phone in the sync loop
